...
public class ChessBoardTest {
    ...
    public void testHas_MaxBoardWidth_of_7() {
        Assert.assertEquals(7, ChessBoard.MAX_BOARD_WIDTH);
    }
    ...
    public void testLimits_The_Number_Of_Pawns() {
        for (int i = 0; i < 10; i++) {
            ...
            int row = 7 / ChessBoard.MAX_BOARD_WIDTH;
            testSubject.Add(pawn, 6 + row, 7 % ChessBoard.MAX_BOARD_WIDTH, PieceColor.BLACK);
            if (row < 1) {
                Assert.assertEquals(6 + row, pawn.getXCoordinate());
                Assert.assertEquals(7 % ChessBoard.MAX_BOARD_WIDTH, pawn.getYCoordinate());
            } else {
