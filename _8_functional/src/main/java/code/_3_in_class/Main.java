...
public class Main {
  ...
  private static void toti_studenti_au_nota_10(List<Student> students) {
    long studentiDeNota10 = students.stream()
            .filter(s -> s.getNota().equals(10))
            .count();
    System.out.println("toti au:" + (studentiDeNota10==students.size()));
  ...
  public static class Student{
    ...
    public void setNume(String nume) throws IllegalAccessException {
      if(nume == null){
        throw new IllegalAccessException("nu poate fi null numele");
      }
      this.nume=nume;
    }
    ...
    public void setNota(Integer nota) throws IllegalAccessException {
      if(nota<1) {
        throw new IllegalAccessException("Nu poate fi null nota");
      }
      this.nota=nota;
