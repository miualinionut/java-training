...
public class MovieDatabase {
    public static Studio[] getMovieDatabase() {
    ...
    public static void getStudioNamesWithMoreTwoMovies() {
        for (Studio studio : getMovieDatabase()) {
            if (studio.film.length > 2)
                System.out.println("Numele studio-ului este: " + " " + studio.nume);
        }
    }

    public static void getStudioNamesWithActorWith2Oscars() {
        for (Studio studio : getMovieDatabase()) {
            for (Film film : studio.film) {
                for (Actor actor : film.actori) {
                    if (film.actori.equals("actor cu 2 oscaruri"))
                        System.out.println("Numele studio-ului este: " + " " + studio.nume);
                }
            }
