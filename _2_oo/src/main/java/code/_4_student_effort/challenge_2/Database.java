...
public class Database {
    ...
    public static void StudioNamesMoreThan2Movies() {
        for (Studio studio : getData())
            if (studio.filme.length > 2)
                System.out.println(studio.nume);
    }

    public static void StudioNamesActorWithName() {
        for (Studio studio : getData())
    ...
    public static void MoviesNameActorAbove50() {
        for (Studio studio : getData())
            for (Film film : studio.filme)
                for (Actor actor : film.actori)
                    if (actor.varsta>55)
...
public static void main (String[] args){
    StudioNamesMoreThan2Movies();
    StudioNamesActorWithName();
    MoviesNameActorAbove50();
}
}
