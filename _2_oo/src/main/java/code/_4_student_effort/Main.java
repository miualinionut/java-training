...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //first requirement of Challenge2 of week 2
        ...
        //second requirement of Challenge2 of week 2
        for (int i = 0; i < studioDatabase.length; i++) {
            Film[] films = studioDatabase[i].getFilme();
            for (int j = 0; j <films.length ; j++) {
                Actor[] actors = films[j].getActori();
                for (int k = 0; k < actors.length; k++) {
                    if (actors[k].getNume() == "actor cu 2 oscaruri")
                        System.out.println(studioDatabase[i]);
                }
            }
        }
        //third requirement of Challenge2 of week 2
        for (int i = 0; i < studioDatabase.length; i++) {
            Film[] films = studioDatabase[i].getFilme();
            for (int j = 0; j < films.length; j++) {
                Actor[] actors = films[j].getActori();
                for (int k = 0; k < actors.length; k++) {
                    if(actors[k].getVarsta() > 50)
                        System.out.println(actors[k]);
                }
            }
        }
