...
public class Main {
  void filme(Scanner s){
    ...
    for (int i = 0; i < studioDatabase.length; i++) {
        if(studioDatabase[i].nrfilme()>=2) System.out.println(studioDatabase[i].getNume());
    }
    for (int i = 0; i < studioDatabase.length; i++) {
      if(studioDatabase[i].getActor("actor cu 2 oscaruri")!=null) System.out.println(studioDatabase[i].getNume());
    }
    for (int i = 0; i < studioDatabase.length; i++) {
      if(studioDatabase[i].getFilm()!=null) System.out.println(studioDatabase[i].getFilm());
    }
  }
  void boxx(Scanner s){
    BoxMatch epic=new BoxMatch(s.nextInt(),s.nextInt(),s.next(),s.nextInt(),s.nextInt(),s.next());
    System.out.println(epic.Fight());
  }
