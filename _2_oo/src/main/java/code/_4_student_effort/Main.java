...
public class Main {
  ...
  public static void main(String[] args) {
   ...
   Film film2 = new Film(1990, "film cu actori fara premii2", new Actor[]{actorFaraPremii02, actorFaraPremii03});
   Film film3 = new Film(1990, "film cu actori fara premii3", new Actor[]{actorFaraPremii02, actorFaraPremii03});
   Film film4 = new Film(1990, "film cu actori de oscar2", new Actor[]{actorOscar2010,actorOscar2018,  actorFaraPremii02});
   ...
   nrOfStudios = studioDatabase.length;
   for(i = 0; i < nrOfStudios; i++){
     if (studioDatabase[i].filme.length > 2)
       System.out.println(studioDatabase[i].nume);
   }

    for(i = 0; i < nrOfStudios; i++){
      if (studioDatabase[i].getFilm("actor cu 2 oscaruri") != null){
          System.out.println(studioDatabase[i].nume);
      }
    }

    for(i = 0; i < nrOfStudios; i++){
        studioDatabase[i].getFilmByAge(50);
    }
