...
public class Challenge04 {
    ...
    static Mono<User> capitalizeOne(Mono<User> mono) {
        return mono.map(user -> new User(user.getFirstName().toUpperCase(),
    ...
    static Flux<User> capitalizeMany(Flux<User> flux) {
        return flux.map(user -> new User(user.getFirstName().toUpperCase(),
                user.getLastName().toUpperCase(),
                user.getUsername().toUpperCase()));
    }
    ...
    static Flux<User> asyncCapitalizeMany(Flux<User> flux) {
        return flux.flatMap(Challenge04::asyncCapitalizeUser);
    }
