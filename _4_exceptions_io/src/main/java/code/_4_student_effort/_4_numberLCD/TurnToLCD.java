package code._4_student_effort._4_numberLCD;
...
public class TurnToLCD {
    public static void main(String[] args) {
        String[] numbersFromFile = new String[0];
        try {
            numbersFromFile = readFromFile("_4_exceptions_io\\_test_files\\in\\input_number.txt");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        for (String number : numbersFromFile) {
            int numberOfChars = number.length();
            writeInFile(getNumberRepresentation(number), numberOfChars, "_4_exceptions_io\\_test_files\\out\\output_number.txt");
        }
    }
