...
public class NumberToLcd {
    ...
    private static final String OUTPUT_file = "_4_exceptions_io\\_test_files\\in\\input_number.txt";

    public static void main(String[] args) {
        String[] numbersFromInputFile = new String[0];
        try {
            numbersFromInputFile = readListOfWords(INPUT_file);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        for (String number : numbersFromInputFile) {
            int numberOfChars = number.length();
            writeInFile(getNumberRepresentationToLCD(number), numberOfChars, OUTPUT_file);
        }
    }

    private static int[][][] getNumberRepresentationToLCD(String number) {
        int numberOfChars = number.length();
        ...
        for (i = 0; i < numberOfChars; i++) {
            if (number.charAt(1) == '1') chars[i] = one;
