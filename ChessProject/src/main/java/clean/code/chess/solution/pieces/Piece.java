...
public abstract class Piece {
    ...
    public void Move(Movement move, int newX, int newY) {
    ...
    public void Capture(Movement move,int newX, int newY) {
        if(move == Movement.CAPTURE){
            throw new UnsupportedOperationException("Need to implement Piece.capture()");
        }else if (this.isValidNewPosition(newX, newY)) {
            this.chessBoard.updatePiecePositionOnChessBoard(this, newX, newY);
        }
        logger.finest(toString());
    }
    ...
    public abstract void Move(int i, int i1);
}
